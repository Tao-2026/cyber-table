...
export async function mountEmulatorApp(container) {
  ...
  const statusText = () => ({ connecting: "CONNECTING TO LOCAL EMULATOR…", synced: "SYNCED · LOCAL EMULATOR", offline: "OFFLINE · WAITING TO RECONNECT", error: "EMULATOR UNAVAILABLE" })[connection];
  ...
  function renderHome(message = "") { shell(`<div><p class="eyebrow">Firebase Emulator</p><h1 class="brand">Cyber <span>Table</span></h1><p class="tagline">Two real anonymous identities · local services only</p></div><div class="hero-art"><span>🤖 💗 🐼 ⭐ 🐰</span></div><div class="actions">${action("CREATE EMULATOR ROOM", "create", "button button-primary")}${action("JOIN WITH CODE", "join", "button button-purple")}${action("BACK TO LOCAL MODE", "local", "button button-ghost")}</div><p class="note">${message}</p>`); }
  function renderJoin() { shell(`<div><p class="eyebrow">Firebase Emulator</p><h1>Join room</h1></div><label class="room-entry">ROOM CODE<input id="fb-code" maxlength="5" placeholder="TST42"></label>${action("JOIN ROOM", "join-submit", "button button-purple")}${action("BACK", "home", "button button-ghost")}<p class="note"></p>`); }
  function renderRoom() {
    if (!room) return;
    const host = room.hostId === api.uid; const match = room.match;
    if (room.status === "playing" && match) return renderMatch(host, match);
    shell(`<div><p class="eyebrow">Lobby ${host ? "· HOST" : ""}</p><h1>Room ${room.roomCode}</h1><p class="tagline">Open another browser session with <code>?backend=emulator</code>.</p></div><div class="room-code-card"><span>ROOM CODE</span><strong>${room.roomCode}</strong></div><div class="player-list">${room.players.map(player => `<div class="player"><span>${player.emoji}</span><strong>${player.playerId === api.uid ? "YOU" : `PLAYER ${player.seat + 1}`}</strong><small>${player.playerId === room.hostId ? "HOST" : "READY"}</small></div>`).join("")}</div>${host ? `<button class="button button-primary" data-fb-action="start" ${room.players.length < 2 ? "disabled" : ""}>START GAME</button>` : `<p class="note">Waiting for host…</p>`}${action("LEAVE VIEW", "home", "button button-ghost")}`);
  }
  ...
  try {
    ...
    const services = await createFirebaseServices({ config: localEmulatorConfig, emulator: true, appName: `cyber-table-${deviceId}` });
